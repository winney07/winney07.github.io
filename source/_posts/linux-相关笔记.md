...
[linux文件创建命令是什么](https://www.php.cn/linux-490062.html)

```
文件创建命令是：1、“vi 文件名”命令，该命令输入保存之后文件才被创建；2、“touch 文件名”命令，该命令用于更改文件或目录的时间，若文件不存在会自动创建；3、“echo 文件内容 > 文件名”命令，该命令会将指定内容写入文件中。
```
